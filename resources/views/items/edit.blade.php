            ...
            <div class="card-body">
                ...
                <form action="{{ route('items.update', $item->id) }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="kingdom_id" class="form-control" id="kingdom_id" required>
                            @foreach($kingdoms as $kingdom)
                                <option value="{{ $kingdom->id }}" {{ $item->kingdom_id == $kingdom->id ? 'selected' : '' }}>{{ $kingdom->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" name="rate" class="form-control" id="rate" value="{{ $item->rate }}" required>
                    </div>

                    <div class="mb-3">
                        <label for="status" class="form-label">Status</label>
                        <input type="text" name="status" class="form-control" id="status" value="{{ $item->status }}" required>
                    </div>
